...
1. **Check for human interrupt** (two-step process, **every beat boundary regardless of mode**):
   - **Step 1 — Check**: Run `bun apps/spectator/cli.ts check-interrupt --dir {playthrough}/spectator` via Bash. Returns instantly (file read only). Parse the JSON. If `interrupted: false`, proceed to step 2.
