...
The team lead receives messages from teammates. In Phase 2, the team lead handles a **smaller set of messages** than Phase 1 — most player-GM communication happens directly.
...
**Note**: In Phase 2, the team lead does NOT collect human input after narrative. The GM sends `[GM_TO_PLAYER]` directly to the human's player teammate, which handles relaying to the human via `[RELAY_TO_HUMAN]`.
...
This skill supports Phase 1 (ephemeral AI players) as a fallback. If the session is running in Phase 1 mode:

- The GM sends `[AWAIT_PLAYERS]` instead of `[GM_TO_PLAYER]` to individual players
- The team lead spawns ephemeral `ai-player-action` Tasks, collects responses, and sends `[PLAYER_RESPONSES]` to the GM
- Auto-journaling uses the legacy `narrative-writer` + `ai-player-journal` agents
- The human interacts directly through the team lead (no human-relay teammate)

See the messaging-protocol skill for `[AWAIT_PLAYERS]` and `[PLAYER_RESPONSES]` formats (Phase 1 only).
