# Party Knowledge

**Campaign**: {Campaign Name}
**Last Updated**: Session {N} ({brief-description})

> This file contains information the ENTIRE PARTY knows. AI players should read this file when invoked.
>
> **AI Players**: This is YOUR source of truth for shared knowledge. If it's not here, your character doesn't know it (unless it's in your personal journal).

## Current Situation

**Location**: {Where the party is}
**Time**: {In-game time}
**What's happening**: {Brief description of current scene/situation}

**Party Formation**:
- {Who is present and general positioning}

## Active Quests

### {Quest Name}
- **Objective**: {What the party is trying to do}
- **Given by**: {Who gave the quest}
- **What we know**: {Information learned so far}
- **Current progress**: {Where we are in the quest}

## NPCs We've Met

| NPC | Who They Are | Our Relationship | Last Interaction |
|-----|--------------|------------------|------------------|
| {Name} | {Role/description} | {Friendly/Neutral/Hostile} | {What happened} |

## Locations We've Visited

### {Location Name}
- **What it is**: {Description}
- **What we found/did there**: {Key events}

## Important Information Learned

<!-- Facts the whole party knows -->

- {Fact 1}
- {Fact 2}

## Party Resources

**Gold**: {Amount} (shared pool or per-character)
**Notable Items**:
- {Item} - held by {Character}

## Recent Session Summary

### Session {N}
{Brief narrative of what happened that session from the party's shared perspective}

## Unresolved Questions

<!-- Mysteries the party is aware of -->

- {Question 1}
- {Question 2}
